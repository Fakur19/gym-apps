...
const getChartDataForRange = async (days) => {
    ...
    const [transactionsData, salesData, checkinsData] = await Promise.all([
    ...
    transactionsData.forEach(item => revenueMap.set(item._id, (revenueMap.get(item._id) || 0) + item.totalRevenue));
    salesData.forEach(item => revenueMap.set(item._id, (revenueMap.get(item._id) || 0) + item.totalRevenue));
...
exports.getDashboardStats = async (req, res) => {
    try {
        ...
        const [todaysTransactions, todaysSales, todaysCheckinsCount, activeMembersCount, newMembersTodayCount, expiringSoonMembers] = await Promise.all([
