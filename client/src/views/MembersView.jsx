  ...
  const printInvoice = (member) => {
    ...
    const invoiceHTML = `
        <div id="invoice-section" class="p-10 bg-white" style="font-family: 'Inter', sans-serif;">
            <div class="flex justify-between items-start mb-12"><div class="flex items-center"><img src="esagym.jpg" alt="Gym Logo" class="w-12 h-12 mr-4"/><div><h1 class="text-2xl font-bold text-gray-800">Esa Gym</h1><p class="text-sm text-gray-500">Jalan Kp Cijeungjing No 101. Rt/Rw 001/023</p><p class="text-sm text-gray-500">Padalarang, Jawa Barat</p></div></div><div class="text-right"><h2 class="text-3xl font-bold text-gray-700">INVOICE</h2><p class="text-sm text-gray-500">#${member._id.slice(-6).toUpperCase()}</p></div></div>
            <div class="grid grid-cols-2 gap-4 mb-12"><div><p class="text-sm font-semibold text-gray-600 mb-1">DITAGIHKAN KEPADA</p><p class="font-medium text-gray-800">${member.name}</p><p class="text-gray-600">${member.email !== undefined ? member.email : ''}</p></div><div class="text-right"><p class="text-sm font-semibold text-gray-600">Tanggal Penerbitan</p><p class="font-medium text-gray-800">${new Date().toLocaleDateString('en-GB')}</p><p class="text-sm font-semibold text-gray-600 mt-2">Status</p><p class="font-medium text-green-500">Sudah Dibayar</p></div></div>
            <table class="w-full text-left mb-12"><thead class="bg-gray-100"><tr><th class="p-3 text-sm font-semibold text-gray-700">DESKRIPSI</th><th class="p-3 text-right text-sm font-semibold text-gray-700">JUMLAH</th></tr></thead><tbody><tr class="border-b border-gray-100"><td class="p-3">Membership: ${latestTransaction.planName}</td><td class="p-3 text-right">${formatCurrency(latestTransaction.price)}</td></tr></tbody></table>
            <div class="flex justify-end mb-12"><div class="w-full max-w-xs"><div class="flex justify-between mb-2"><p class="text-gray-600">Subtotal</p><p class="text-gray-800">${formatCurrency(latestTransaction.price)}</p></div><div class="flex justify-between mb-2"><p class="text-gray-600">Pajak</p><p class="text-gray-800">${formatCurrency(0)}</p></div><div class="border-t border-gray-200 my-2"></div><div class="flex justify-between"><p class="font-bold text-gray-800">Total</p><p class="font-bold text-blue-600 text-lg">${formatCurrency(latestTransaction.price)}</p></div></div></div>
            <div class="text-center text-sm text-gray-500 border-t pt-6"><p class="font-semibold">Terima Kasih!</p><p>Jika Anda memiliki pertanyaan, silakan hubungi kami di 0813-9487-3908.</p></div>
        </div>`;
